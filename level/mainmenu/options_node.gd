...
onready var buttons = [$gameplay, $controls, $graphics,$audio ,$back]
...
func _physics_process(delta: float) -> void:
	if menuscene.current_menu_state == menuscene.menu_states.OPTIONS:
		update()
...
#			buttons[i].get_node("text").rect_position.y += (-8 - buttons[i].get_node("text").rect_position.y) * 0.9
		...
		if Input.is_action_just_pressed("CLICK") or Input.is_action_just_released("ui_accept"):
			...
			$menu_cycle_sfx.play()
			if position.distance_squared_to(Vector2(32, 44)) < 300:
				if index == 0:
					menuscene.current_menu_state = menuscene.menu_states.SUB_OPTIONS_GAMEPLAY
				if index == 4:
