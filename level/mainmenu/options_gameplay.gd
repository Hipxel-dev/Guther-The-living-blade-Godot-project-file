...
onready var buttons = [$camera_speed, $camera_panning, $hud_visibility, $hud_padding, $back]
...
func _physics_process(delta: float) -> void:
	if menuscene.current_menu_state == menuscene.menu_states.SUB_OPTIONS_GAMEPLAY:
		update()
		position = position.linear_interpolate(Vector2(32, 44), delta * 15)
...
#			buttons[i].get_node("text").rect_position.y += (-8 - buttons[i].get_node("text").rect_position.y) * 0.9
		...
		if Input.is_action_just_pressed("CLICK") or Input.is_action_just_released("ui_accept"):
			...
			$menu_cycle_sfx.play()
			if index == 4 and position.distance_squared_to(Vector2(32, 44)) < 300:
