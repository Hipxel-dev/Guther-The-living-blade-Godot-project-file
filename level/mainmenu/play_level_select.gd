...
onready var buttons = [$campaign, $survival, $back]
...
func _physics_process(delta: float) -> void:
	if menuscene.current_menu_state == menuscene.menu_states.SUB_PLAY_LEVEL_SELECT:
		update()
...
#			buttons[i].get_node("text").rect_position.y += (-8 - buttons[i].get_node("text").rect_position.y) * 0.9
		...
		if Input.is_action_just_pressed("CLICK") or Input.is_action_just_released("ui_accept"):
			...
			if position.distance_squared_to(Vector2(32, 32)) < 300:
				if index == 0:
					menuscene.current_menu_state = menuscene.menu_states.SUB_CHAPTER_SELECT
				if index == 0:
					get_tree().change_scene("res://level/arena1.tscn")
