...
onready var buttons = [$play, $options, $extras, $quit, $none]
...
func do_stuff_of_the_selected_button(delta): 
	for i in range(buttons.size()):
		buttons[index].get_node("text").rect_position.x = 34 + (abs(sin(OS.get_ticks_msec() * delta * 5) * (global.shake * 25))) + rand_range(-global.shake,global.shake) * 15
		
		buttons[index].modulate = Color8(255,0,0)
		
	if Input.is_action_just_pressed("ui_down"):
		
		if index < buttons.size() - 2:
			index += 1
		else:
			index = 0
	...
	if Input.is_action_just_pressed("ui_up"):
		if index > 0:
			...
			index = buttons.size() - 2
		get_tree().get_root().warp_mouse(buttons[index].global_position + Vector2(-16, 0))
		
func _physics_process(delta: float) -> void:
	if menuscene.current_menu_state == menuscene.menu_states.MAIN:
		update()
		position = position.linear_interpolate(Vector2(32, 88), delta * 15)
...
#			buttons[i].get_node("text").rect_position.y += (-8 - buttons[i].get_node("text").rect_position.y) * 0.9
		...
		if Input.is_action_just_pressed("ui_cancel"):
			menuscene.current_menu_state = menuscene.menu_states.PRESS_START
		...
		if Input.is_action_just_pressed("CLICK") or Input.is_action_just_released("ui_accept"):
			...
			$menu_cycle_sfx.play()
			if position.distance_squared_to(Vector2(32, 88)) < 300:
				if index == 0:
					menuscene.current_menu_state = menuscene.menu_states.SUB_PLAY_LEVEL_SELECT
				if index == 1:
					menuscene.current_menu_state = menuscene.menu_states.OPTIONS
				if index == 2:
					menuscene.current_menu_state = menuscene.menu_states.EXTRAS
				if index == 3:
					menuscene.current_menu_state = menuscene.menu_states.QUIT
...
#		buttons[i].position.x = 1 * sin(i * 5 + OS.get_ticks_msec() * 0.01)
#		buttons[i].position += get_local_mouse_position()
